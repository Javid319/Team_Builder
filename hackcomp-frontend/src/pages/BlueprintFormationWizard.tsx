...
const STEPS = ['Domain Interests', 'Choose Goal', 'Blueprint Details', 'Team Structure', 'Review & Create'];
type Step = 1 | 2 | 3 | 4 | 5;
type TeamGoal = 'recruit' | 'join' | null;
...
const Stepper = ({ current, goal }: { current: Step, goal: TeamGoal }) => {
  // If goal is 'join', we don't show steps 3-5
  const displaySteps = goal === 'join' ? STEPS.slice(0, 2) : STEPS;
...
const BlueprintFormationWizard = () => {
  ...
  const ROLE_SKILLS: Record<string, string[]> = {
    'backend_developer': ['python', 'nodejs', 'java', 'go', 'ruby', 'php', 'c#', 'sql', 'postgresql', 'mysql', 'mongodb', 'redis', 'docker', 'kubernetes', 'aws', 'restapis', 'graphql', 'django', 'fastapi', 'spring'],
    'frontend_developer': ['html', 'css', 'javascript', 'typescript', 'react', 'vue', 'angular', 'svelte', 'nextjs', 'tailwindcss', 'bootstrap', 'sass', 'redux', 'webpack', 'vite', 'webaccessibility', 'figma'],
    'fullstack_developer': ['javascript', 'typescript', 'react', 'nodejs', 'python', 'django', 'postgresql', 'mongodb', 'docker', 'aws', 'html', 'css', 'nextjs', 'express', 'graphql'],
    'ml_engineer': ['python', 'pytorch', 'tensorflow', 'scikitlearn', 'pandas', 'numpy', 'keras', 'opencv', 'transformers', 'langchain', 'mlops', 'sql', 'spark', 'hadoop'],
    'cloud_engineer': ['aws', 'azure', 'gcp', 'terraform', 'kubernetes', 'docker', 'linux', 'bash', 'python', 'networking', 'security', 'ci/cd', 'ansible'],
    'devops_engineer': ['docker', 'kubernetes', 'jenkins', 'githubactions', 'gitlabci', 'terraform', 'ansible', 'linux', 'bash', 'python', 'aws', 'prometheus', 'grafana'],
    'mobile_developer': ['swift', 'kotlin', 'java', 'objective-c', 'reactnative', 'flutter', 'dart', 'xcode', 'androidstudio', 'firebase', 'sqlite'],
    'data_engineer': ['python', 'sql', 'spark', 'hadoop', 'kafka', 'airflow', 'dbt', 'snowflake', 'bigquery', 'redshift', 'postgresql', 'aws', 'gcp', 'dataanalysis'],
    'cybersecurity': ['python', 'bash', 'linux', 'kali', 'wireshark', 'nmap', 'metasploit', 'burpsuite', 'security', 'cryptography', 'networksecurity', 'pentesting', 'owasp'],
    'other': ['projectmanagement', 'agile', 'scrum', 'jira', 'confluence', 'communication', 'leadership', 'design', 'ui/ux', 'marketing', 'sales', 'contentcreation']
  };
  ...
  const addSlot = () => {
    ...
    setSlots((prev) => [...prev, { id: crypto.randomUUID(), role: '', skills: [] }]);
  };
  ...
  const canProceed = () => {
    ...
    if (step === 3) return teamName.trim().length > 0;
    if (step === 4) return slots.length > 0 && slots.every(s => s.role !== '');
    return true;
  };
  ...
  const next = () => {
    ...
    if (step === 2 && goal === 'join') {
      navigate(`/blueprints/browse?hackathonId=${hackathon?.id}&domains=${domains.join(',')}`);
      return;
    }
      ...
      <div className="main-workspace fade-in">
        <div className="team-success">
          ...
          <div className="flex wrap gap-2" style={{ marginTop: 20 }}>
            <Link to={`/blueprints/${createdBlueprint.id}/dashboard`} className="btn btn-primary btn-sm"><ArrowRight size={14} /> Go to Dashboard</Link>
            <Link to={`/blueprints/${createdBlueprint.id}/recommendations`} className="btn btn-ghost btn-sm">Find Candidates</Link>
          </div>
        </div>
      </div>
    );
    ...
    <div className="main-workspace fade-in">
      ...
      <div className="team-create-head">
        <div>
          ...
          <h1 className="team-create-title">Formation Wizard</h1>
          <p className="text-subtle">{hackathon.title}</p>
        </div>
        <Stepper current={step} goal={goal} />
      </div>

      {step === 1 && (
        <section className="team-panel">
          <h2 className="hack-detail-heading">Select Domain Interests</h2>
          <p className="hack-detail-lede">Choose which domains you are interested in.</p>
      ...
      {step === 2 && (
        <section className="team-panel fade-in">
          <h2 className="hack-detail-heading">Choose Your Goal</h2>
          <p className="hack-detail-lede">How do you want to build your team?</p>
          <div className="goal-grid" style={{ marginTop: 20 }}>
            <button
              type="button"
              className={`goal-card ${goal === 'recruit' ? 'is-active' : ''}`}
              onClick={() => setGoal('recruit')}
            >
              <div className="goal-card-icon">
                <Users size={26} />
              </div>
              <h3 className="goal-card-title">Looking for Members</h3>
              <p className="goal-card-desc">Create a blueprint, define roles, and invite candidates.</p>
            </button>
            <button
              type="button"
              className={`goal-card ${goal === 'join' ? 'is-active' : ''}`}
              onClick={() => setGoal('join')}
            >
              <div className="goal-card-icon">
                <Handshake size={26} />
              </div>
              <h3 className="goal-card-title">Looking to Join a Team</h3>
              <p className="goal-card-desc">Browse open blueprints and request to join them.</p>
            </button>
          </div>
        </section>
      )}
      ...
      {step === 4 && (
        <section className="team-panel fade-in">
          <div className="flex items-center justify-between" style={{ marginBottom: '1rem' }}>
             ...
             <button type="button" className="btn btn-outline btn-sm" onClick={addSlot} disabled={slots.length >= 4}>
          ...
          {slots.length === 0 ? (
            ...
            <div className="team-form" style={{ maxWidth: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', alignItems: 'start' }}>
                ...
                <div key={slot.id} className="team-review-block" style={{ position: 'relative', background: 'var(--surface)' }}>
                  ...
                  <label className="form-group">
                    ...
                    <select 
                      className="form-control" 
                      value={slot.role}
                      onChange={(e) => updateSlot(slot.id, { role: e.target.value as CandidateRole, skills: [] })}
                    >
                  ...
                  <div className="form-group">
                    ...
                    {slot.role ? (
                      <div className="team-filter-chips" style={{ maxHeight: '150px', overflowY: 'auto', padding: '4px' }}>
                        {getSkillsForRole(slot.role).map(skill => {
                      ...
                      <div className="text-subtle text-sm" style={{ padding: '8px 0' }}>
                        Please select a role first to see preferred skills.
                      </div>
                    )}
                  </div>
                </div>
...
export default BlueprintFormationWizard;
